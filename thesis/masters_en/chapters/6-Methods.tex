...
\subsection{Sequential preparation routine}
\label{sec:frqi-seq}
The circuit builds \Cref{eq:frqi-state} one address at a time, repeating the same \emph{match--rotate--uncompute} pattern for every pixel; only the rotation angle changes between iterations. The routine below matches files \texttt{src/frqi.py} and \texttt{src/improved.py}, and the structural variants of \Cref{sec:struct-prep} differ only in how the multi-controlled $X$ layers are synthesized, not in the output state.
...
The same logical FRQI state is realized with two Qiskit \texttt{MCX} gate modes that trade qubits against gate count:
\begin{itemize}
...
\end{itemize}
Full naive transpilation runs only for $4\times 4$. For $8\times 8$ and $16\times 16$, the table reports \texttt{struct\_naive\_slice\_scaled}: it transpiles one representative pixel slice and multiplies the depth and CX by $N_{\mathrm{pix}}$.

\subsection{MCX gates and address matching}
\label{sec:mcx-explained}
The Multi-Controlled NOT gate (MCX) is the building block that makes FRQI's address matching possible. A plain NOT gate (Pauli $X$) flips one qubit; a Controlled-NOT (CX) flips its target only when one control is $\ket{1}$; and a Multi-Controlled NOT flips the target only when \textbf{all} of its controls are $\ket{1}$ at once. With $k$ controls it is written $C^{k}X$, and the two-control case is the Toffoli gate.
...
\subsection{Schematics: naive versus v-chain circuit}
\Cref{fig:toy-naive-vchain} contrasts a three-qubit address register plus one color qubit in the naive \texttt{noancilla} view (one conceptual multi-controlled rotation block) against an ancilla-assisted v-chain slice (MCX ladder, controlled $R_y$, uncompute). \Cref{fig:block-frqi-prep} then places the same per-pixel pattern inside the full block-level preparation loop. Real FRQI circuits repeat the inner pattern for every pixel address. Drawings come from \texttt{scripts/toy\_and\_block\_level\_diagrams.py} and are stored under \path{thesis/design/figures/}.
...
\subsection{Schematics: Qiskit \texttt{MCX} v-chain gate}
\Cref{fig:stub-mcx} shows a minimal Qiskit circuit illustrating how a MCX expands in v-chain mode before basis-gate transpilation.

\begin{figure}[H]
\centering
\includegraphics[width=0.55\textwidth]{stub_mcx_vchain.png}
\caption{\texttt{MCX gate} with \texttt{mode="v-chain"} on a few qubits (\texttt{outputs/stub\_mcx\_vchain.png}, from \texttt{scripts/qiskit\_stub\_draw.py}).}
\label{fig:stub-mcx}
\end{figure}
...
\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=*]
  ...
  \item \textbf{Walsh--Hadamard transform.} Apply a fast Walsh--Hadamard transform (FWHT) along rows and then columns. The FWHT is a close cousin of the Fourier transform but uses only additions and subtractions: it repeatedly forms sums and differences of pairs of values, so it measures how strongly the image changes between neighbouring positions. in other words, sharp changes (edges) produce large coefficients; flat regions produce small ones.
  \item \textbf{Drop the sign.} Take the absolute value of the coefficients in order to have a contrast map, while each change keeps its strength.
  \item \textbf{Inverse transform.} Apply the inverse FWHT to bring the contrast map back to the spatial layout, so high values sit where the image is varying the most.
...
\begin{table}[htbp]
\centering
\caption{Synthetic noise grid (each scale multiplies the baseline channel strengths).}
...
\caption{Ideal FRQI+QHED baseline pipeline (\srccode{scripts/frqi_qhed_sobel.py}).}
...
\caption{E1 ideal baseline flow: a single clean image is encoded, decoded, edge-detected, and scored, with the help of transpiled resource metrics.}
...
\begin{table}[H]\centering\footnotesize
\caption{Structural resource pipeline (\srccode{scripts/frqi_structural_resources.py}).}
...
\caption{E2 structural resource flow: both synthesis modes are transpiled to \texttt{\{cx,rz,sx\}} and their CX/depth logged against image size.}
...
\begin{table}[H]\centering\footnotesize
\caption{Noisy FRQI density-matrix pipeline (\srccode{src/noise_models.py::noisy_frqi_metrics_row}, driven by \srccode{scripts/noisy_frqi_sweep.py}).}
...
\begin{table}[H]\centering\footnotesize
\caption{Noisy-reconstruction edge pipeline (\srccode{scripts/noisy_recon_qhed_edge_metrics.py}).}
...
\caption{E4 noisy-edge flow: QHED runs on the noisy reconstruction and the resulting edge map is compared with the classical Sobel reference.}
...
\begin{table}[H]\centering\footnotesize
\caption{Readout mitigation pipeline (\srccode{scripts/readout_mitigation_shot_sweep.py}).}
...
\label{sec:pipe-e6}
\emph{Purpose: validate CSVs and turn them into the thesis tables and figures.}

\begin{table}[H]\centering\footnotesize
\caption{Results-ingestion pipeline (\srccode{scripts/build_results_tables.py}).}
...
Datasets live under \texttt{data/test\_images/} as \texttt{test\_4x4.npy}, \texttt{test\_8x8.npy}, and \texttt{test\_16x16.npy}. The Python environment is pinned in \texttt{requirements.txt} (Qiskit~1.x, Qiskit Aer, NumPy, SciPy, pandas, Matplotlib, scikit-image, etc.). The individual experiments and their end-to-end code paths are detailed in \Cref{sec:pipelines}; \Cref{tab:pipe-overview} maps each one to its driver script and primary artifact.

\subsection{Repository layout}
In \Cref{fig:repo-tree}, the repository layout is outlined with comments for the most important content inside of it.
...
+-- LICENSE
\end{lstlisting}
\caption{The Repository layout: it contains key directories such as \texttt{src/} modules and \texttt{scripts/} drivers. The full module-to-script mapping is in \Cref{tab:repo-map}.}
\label{fig:repo-tree}
\end{figure}
